...
const deactivateByPluginId = new Map<string, () => void>();
...
export async function loadPluginBackend(
  ...
  const { context, dispatch, deactivate } = createPluginContext(manifest.slug, pluginId, db, io);

  const backendUrl = `file://${backendEntry}?t=${Date.now()}`;
  const pluginModule = await import(backendUrl);
  ...
  deactivateByPluginId.set(pluginId, deactivate);
}

export function unloadPlugin(slug: string, pluginId: string) {
  const deactivate = deactivateByPluginId.get(pluginId);
  if (deactivate) {
    deactivate();
  }
  loadedPlugins.delete(slug);
  dispatchByPluginId.delete(pluginId);
  deactivateByPluginId.delete(pluginId);
}

export async function uninstallPlugin(pluginId: string, db: Knex, io?: Server) {
  ...
  if (io) {
    io.emit('plugin:uninstalled', { slug: plugin.slug });
  }
}
