...
export function createPluginContext(
    ...
    lobby: {
      ...
      async setStatus(lobbyId, status) {
        ...
        if (status === 'beendet' || status === 'geschlossen') {
          ...
          // Offene Game Sessions beenden, damit ended_at gesetzt ist
          // (noetig fuer Top-Movers-Query und korrekte scoreChange-Zuordnung)
          await db('game_sessions')
            .where('lobby_id', lobbyId)
            .whereNull('ended_at')
  ...
  return { context, dispatch, deactivate };
}
